...
1. **検出タイプの選択**
   - アプリケーション上部のプルダウンで「文章形式」または「表形式」を選択
   - 文章形式：標準的なテキスト認識に最適化
   - 表形式：表やドキュメントレイアウト認識に最適化

2. **範囲選択**
   - アプリケーション内の「範囲選択」ボタンをクリック
...
1. **ファイル選択**
   - アプリケーション内の「ファイル選択」ボタンをクリック
   - 抽出したいPDFファイルを選択（複数ファイル対応）

2. **自動OCR処理**
   - 各ページが自動的にGoogle Cloud Vision APIで処理されます
   - ページごとの処理結果が連結されてテキストエリアに表示されます

**PDF処理の設定**:
- `config.ini`の`[PDF]`セクション内で`max_pages`を設定可能(デフォルトは20ページ)
- 大容量PDFの処理が必要な場合は、この値を調整してください
...
6. **モード切り替え**
   - プルダウンメニューで「追記モード」「上書きモード」を切り替え
   - 次のOCR結果またはPDF抽出の挿入方法を制御
...
1. 抽出したい日本語文字の領域を選択
2. 認識されたテキストで「読点除去」「改行除去」等を実行
3. 結果を自動でクリップボードコピーまたはファイル保存

#### PDFからのテキスト抽出

スキャンされた日本語PDFから一括でテキストを抽出する場合:

1. 「ファイル選択」ボタンをクリック
2. 対象のPDFファイルを選択（複数選択可）
3. 自動的に全ページがOCR処理され、結果が連結されて表示
4. 必要に応じてテキスト編集機能で整形
5. 結果をファイル出力または全文コピー
...
#### 2. ビジネスロジック層 (service/ + external_service/)
- **VisionOCRService** (`external_service/vision_ocr_service.py`): OCR処理エンジン
  - Google Cloud Vision APIとの統合
  - 画像→テキスト変換

- **PDF処理** (`service/pdf_processor.py`): PDF操作
  - PDFファイルの読み込み
  - ページごとのOCR処理
  - エラーハンドリング

- **ファイル操作** (`service/file_saver.py`): ファイルI/O
  - テキスト保存とダイアログ管理

- **テキスト処理** (`service/text_widget_utils.py`): Tkinter操作とテキスト変換
  - テキストウィジェットへの追記/上書き処理
  - 読点・句点・改行・空白の除去

#### 3. ユーティリティ層 (utils/)
- **ConfigManager** (`config_manager.py`): 設定管理
  - config.iniから設定読み込み（ウィンドウ、スクリーンキャプチャ、PDF処理、OCR検出タイプの設定など）
  - OCR検出タイプ（`text_detection` / `document_text_detection`）の取得・管理
  - PDF最大ページ数（`max_pages`）の取得・管理
  - PyInstaller互換のリソース取得（開発環境では`utils/`、PyInstaller環境では`sys._MEIPASS`から設定ファイルを読み込む）

- **環境変数管理** (`env_loader.py`): 認証情報ロード
  - `.env`ファイル解析とパス解決
  - PyInstaller環境対応（ビルド時と開発時の`.env`ファイルパスを自動判定）
  - サービスアカウント認証情報初期化
...
# 特定のテストファイル実行
python -m pytest tests/services/test_vision_ocr_service.py -v

# 特定のテスト実行
python -m pytest tests/services/test_vision_ocr_service.py::test_perform_ocr -v
```
...
2. Google Cloud Consoleでサービスアカウントが有効か確認
