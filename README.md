...
VisionOCR/
├── app/                           # プレゼンテーション層
│   ├── app_window.py             # メインUIウィンドウ
│   └── app_screen_capture.py     # スクリーン領域選択・キャプチャ
├── external_service/             # 外部API連携層
│   └── vision_ocr_service.py     # Google Cloud Vision API統合
├── service/                       # ビジネスロジック層
│   ├── file_saver.py            # ファイル保存処理
│   ├── text_widget_utils.py      # テキスト処理・UI操作
│   └── pdf_processor.py          # PDF処理エンジン
├── utils/                         # ユーティリティ層
│   ├── config_manager.py         # 設定ファイル管理
│   ├── constants.py              # UI定数・メッセージ定義
│   ├── env_loader.py             # 環境変数ロード
│   └── log_rotation.py           # ログローテーション
├── widgets/                       # UIコンポーネント
│   └── button_factory.py         # ボタン生成ファクトリ
├── tests/                         # テスト群
├── scripts/                       # ユーティリティスクリプト
│   └── json_minifier.py          # JSON最小化スクリプト
├── main.py                        # アプリケーション起動エントリポイント
├── build.py                       # PyInstaller用ビルドスクリプト
